...
<div class="info">
  ...
  <ul>
    ...
    <li v-for="(item, index) in attrs" :key="index" v-if="item.id!=2" @click="onChangeInfo('attrs', item)">
      <span>{{item.attr}}</span>
      <span>{{item.selectIdx == -1 ? '请选择' : item.value[item.selectIdx].text}}</span>
    </li>
  </ul>
  <button type="button" class="submit-info" @click="submitInfo">提交信息</button>
  <van-popup v-model="showPopup" position="bottom">
    ...
    <van-datetime-picker v-else-if="changeTag == 'birthday'" v-model="birthday.currentDate" type="date" :min-date="birthday.minDate" :max-date="birthday.maxDate" @cancel="onCancel" @confirm="onConfirmBirthday" />
    <van-area v-else-if="changeTag == 'address'" :area-list="areaList" :columns-num="3" @change="onChangeAddress" @cancel="onCancel" @confirm="onConfirmAddress" :value="userInfo.district_id + ''" />
    <van-picker v-if="changeTag == 'attrs'" :columns="attrsColums" :default-index="attrsSelectIdx" show-toolbar @cancel="onCancel" @confirm="onConfirmAttrs" />
  </van-popup>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    onConfirmBirthday(val) {
      ...
      // });
      this.userInfo.birthday = val.format("yyyy-MM-dd")
    },
    ...
    // 提交信息
    submitInfo() {
      this.$http.put("/api/app/user", this.userInfo).then(() => {
        this.$toast("数据更新成功");
         if (this.app) {
         this.$pub.appAction("setMessageComplete");
         }
        this.$router.replace({
          name: "Person"
        });
